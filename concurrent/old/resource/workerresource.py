...
from .errors import *
from .messenger import Messenger, BaseMessage, MessagePriority
...
from .workerprocess import BaseWorkerProcess, SimpleWorkerProcess, MessengerWorkerProcess
...
class WorkerResource:
    ...
    @classmethod
    def start_simpleworker(cls, 
        worker_target: typing.Callable, 
        message_prioritizer: typing.Callable[[BaseMessage], MessagePriority],
        verbose: bool = False,
        method: str = 'forkserver',
    ) -> WorkerResource:
        resource: cls = cls.new_resource(
            SimpleWorkerProcess,
            worker_target,
            message_prioritizer,
            verbose,
    ...
    def new_resource(cls, 
        ...
        resource_pipe, process_pipe = Pipe(duplex=True)
        resource: cls = cls(
            messenger = Messenger(
                pipe = resource_pipe, 
    ...
    def execute_immediately(self):
        self.messenger.send_message(StatusRequestMessage(self.get_next_id()))
...
#
    ...
    def join(self, check_alive=True):
        ...
        try:
            self.messenger.send(SigCloseMessage())
        except BrokenPipeError:
            pass
